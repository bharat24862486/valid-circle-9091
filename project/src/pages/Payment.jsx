...
import { GET_LOGOUT_SUCCESS } from '../redux/AuthReducer/ActionTypes'
...
const Payment = () => {
 ...
 const val = JSON.parse(localStorage.getItem('cartAmount'))
   ...
   <Spinner
    ...
    <VStack width={"27%"} p={"30px"} borderRadius={"10px"} spacing={8}   bgColor={"rgba(229,229,229,0.3)"}>
           ...
           <HStack spacing={"24px"}>
             ...
             <Box py={"15px"} w="full">
             <Button colorScheme={"facebook"} w="100%" onClick={handlePayment}>Pay {val} </Button>
             </Box>
           </VStack>
          </FormControl>
